...
static const uint8_t spp_adv_data[23] = {
    ...
    0x0F,0x09, 'R', 'O', 'O', 'M','_', 'S', 'P', 'P', '_', 'S', 'E', 'R','V', 'E', 'R'
};
...
static uint16_t handle_to_data=0;
static bool enable_data_ntf = false;
static bool is_connected = false;
static esp_bd_addr_t spp_remote_bda = {0x0,};

static uint16_t spp_handle_table[SPP_IDX_NB];//array for handles to all spp attrs
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
    	...
    	case ESP_GATTS_WRITE_EVT: {
            ...
            handle_to_data=p_data->write.handle;
...
void printData(void){
    const uint8_t *value=NULL;
    uint16_t length=0;
    esp_gatt_status_t status=esp_ble_gatts_get_attr_value(handle_to_data,&length,&value);
    uint8_t *p_data=value;
    esp_log_buffer_char(GATTS_TABLE_TAG,(char*)p_data,length);
}
void app_main(void)
{
    ...
    spp_task_init();
    while(!data_written){
        ESP_LOGI(GATTS_TABLE_TAG,"waiting to write data");
