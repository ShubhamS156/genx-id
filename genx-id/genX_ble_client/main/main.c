...
/*@Brief
 * function to store info of prov'd devices
 */
...
{
    uint8_t match[2] = { 0x32, 0x10 }; //get the match data from smartphone
    esp_err_t err;
    ...
    err = esp_ble_mesh_provisioner_set_dev_uuid_match(match, sizeof(match), 0x0, false);//imp function
    if (err != ESP_OK) {
